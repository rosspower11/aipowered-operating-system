...
description: Turn a handful of someone's real posts and emails into a written Voice DNA document, so every piece Claude writes for them sounds like they wrote it. Use this skill whenever the user wants to "capture my voice", "build my voice DNA", "make Claude sound like me", "create a voice guide", "write in my style", "stop sounding like AI", "analyse my writing", or drops in samples of their own posts, emails, captions or scripts and asks Claude to learn how they sound. Also trigger when the user complains that AI copy "doesn't sound like me" and wants a fix, or asks for a reusable style guide built from their own writing. Do not use it to write a single piece of content (use a drafting skill) or to enforce an existing voice guide that already exists.
---
